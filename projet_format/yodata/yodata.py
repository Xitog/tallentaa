import pygame
pygame.init()

def byte_to_pretty_s(bytes):
    return ' '.join('{:02x}'.format(byte) for byte in bytes)
    
    
def byte_to_s(bytes):
    return str(bytes, 'ascii')

    
def byte_to_i(bytes, order='little'):
    return int.from_bytes(bytes, byteorder=order)

surfaces = []

palette = [
	,
]

with open("D:\Autres\Star Wars - Yoda Stories (1997)\yodesk.dta", "rb") as yoda_data_file:
    go_on = True
    nb = 0
    while go_on:
        section_start = nb
        bytes = yoda_data_file.read(4)
        nb += 4
        print(byte_to_pretty_s(bytes))
        section = byte_to_s(bytes)
        if section == "VERS":
            version_major = byte_to_i(yoda_data_file.read(2), 'big')
            version_minor = byte_to_i(yoda_data_file.read(2), 'big')
            nb += 4
            print('Version: ', str(version_major) + '.' + str(version_minor))
        elif section in ['STUP', 'SNDS', 'TILE', 'PUZ2', 'CHAR', 'CHWP', 'CAUX', 'TNAM']:
            size = byte_to_i(yoda_data_file.read(4))
            nb += 4
            print('Size of ' + section + ' @' + str(section_start) + ':', size)
            if section == 'TILE':
                nb_tex = 1
                nb_read = 0
                print('Nb tiles =', size // 1028)
                for i in range(0, size // 1028):
                    print('Tiles n', nb_tex, size // 1028)
                    flags = yoda_data_file.read(4) # byte_to_s(yoda_data_file.read(4))
                    nb_read += 4
                    s = pygame.Surface((32, 32), pygame.SRCALPHA)
                    s.lock()
                    for j in range(0, 1024):
                        data = byte_to_i(yoda_data_file.read(1))
                        nb_read += 1
                        # blue =  data & 255
                        # green = (data >> 8) & 255
                        # red =   (data >> 16) & 255
                        r = palette[data * 4 + 2];
                        g = palette[data * 4 + 1];
                        b = palette[data * 4 + 0];
                        # print(data, r, g, b) r != 0 or g != 0 or b != 0
                        c = pygame.Color(r, g, b, 255) if data != 0 else pygame.Color(0, 0, 0, 0)
                        s.set_at((j % 32, j // 32), c)
                    s.unlock()
                    pygame.image.save(s, 'output\\' + str(nb_tex) + '.png') # + flags + '.png')
                    nb_tex += 1
                print('Tiles read:', nb_tex, 'Bytes read:', nb_read)
            else:
                data = yoda_data_file.read(size)
                nb += size
        elif section == 'ZONE':
            #yoda_data_file.read(2218234)
            #nb += 2218234
            nb = byte_to_i(yoda_data_file.read(2))
            print('Size of ' + section + ' @' + str(section_start) + ': unknown [' + str(nb) + ' zones]')
            for i in range(0, nb):
                yoda_data_file.read(2)
                size = byte_to_i(yoda_data_file.read(4))
                yoda_data_file.read(size)
        elif section == 'ENDF':
            go_on = False
        else:
            raise Exception("This section is not handled: " + section)

pygame.quit()
print('Finished')
