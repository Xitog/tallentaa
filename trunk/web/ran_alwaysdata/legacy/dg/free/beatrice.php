                        ...
                        switch($content) {
                            case "voc":
                                if (isset($_GET['cat']) and check($_GET['cat'])) {
                                    ...
                                    $cat = $_GET['cat'];
                                    $requete = "SELECT count(*) as c FROM words WHERE langue = 'italien' and tag = '" . $cat ."'";
                                    if ($resultat = get($requete)) {
                                        $arr = mysql_fetch_assoc ($resultat);
                                        echo '<p>' . $arr[c] . ' termes.</p>';
                                    }
                                    $requete = "SELECT id, classe, modifier, meaning, subtag FROM words WHERE langue = 'italien' and tag = '" . $cat . "' ORDER BY subtag ASC, `order` ASC, classe ASC, id ASC";
                                    if ($resultat = get($requete)) {
                                        $subtag = '';
                                        echo '<table>';
                                        while ($arr = mysql_fetch_assoc ($resultat)) {
                                            ...
                                            echo '<tr><td>' . fword_it($arr['id'], $arr['classe'], $arr['modifier']) . '</td><td>' . $arr['meaning'] . '</td></tr>';
                                        }
                                        echo '</table>';
                                    }
                                }
                            ...
                            case "search":
                                if (isset($_GET['item']) and check($_GET['item'])) {
                                    $item = $_GET['item'];
                                    $requete = "SELECT id, classe, modifier, meaning FROM words WHERE id like '" . $item . "' ORDER BY id ASC";
                            ...
                            case "conjugate":
                                if (isset($_GET['item']) and check($_GET['item'])) {
                                    $item = $_GET['item'];
                                    $requete = "SELECT verbs.id, tenses.tense as ptense, sg_1,	sg_2, sg_3, pl_1, pl_2, pl_3, spe FROM verbs, tenses WHERE verbs.id = '" . $item . "' AND tenses.id = verbs.tense ORDER BY id ASC";
                                    $resultat = get($requete);
                                    if (mysql_num_rows($resultat) == 0) {
                                        ...
                                        while ($arr = mysql_fetch_assoc ($resultat)) {
                                            if (is_null($arr['spe'])) {
                                                echo '<p class="text">' . ucfirst($arr['ptense']) . '</p>';
                                                echo '<table>';
                                                echo '<tr><td>io</td><td>' . $arr['sg_1'] .  '</td></tr>';
                                                echo '<tr><td>tu</td><td>' . $arr['sg_2'] .  '</td></tr>';
                                                echo '<tr><td>lui, lei</td><td>' . $arr['sg_3'] .  '</td></tr>';
                                                echo '<tr><td>voi</td><td>' . $arr['pl_1'] .  '</td></tr>';
                                                echo '<tr><td>noi</td><td>' . $arr['pl_2'] .  '</td></tr>';
                                                echo '<tr><td>loro</td><td>' . $arr['pl_3'] .  '</td></tr>';
                                                echo '</table>';
                                            }  else {
                                                echo '<p class="text">' . ucfirst($arr['ptense']) . '</p>';
                                                echo '<table>';
                                                echo '<tr><td>' . $arr['spe'] .  '</td></tr>';
                                                echo '</table>';
                                            }
                                        }
                                    }
                                ...
                                break;
                            case "tests":
                                $requete = "SELECT id, classe, modifier, meaning FROM words ORDER BY id ASC";
                                $resultat = get($requete);
                                $res = array();
                                while ($arr = mysql_fetch_assoc ($resultat)) {
                                    $res[]=$arr;
                                ...
                                foreach ($rand_keys as $key) {
                                    $arr = $res[$key];
                                    echo '<tr><td>'.$i.'</td><td>' . fword_it($arr['id'], $arr['classe'], $arr['modifier']) . '</td><td>' . $arr['meaning'] . '</td></tr>';$input[$rand_keys[0]] . "\n";
                                    $i+=1;
                                }
                                echo '</table>';
                                break;
                            default:
                                $requete = "SELECT id, title, content FROM chunks WHERE id = '" . $content . "'";
                                if ($resultat = get($requete)) {
