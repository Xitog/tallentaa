            ...
            <tr>
                ...
                <td class="middle">
                    ...
                    if (isset($_GET['content']))
                    {
                        ...
                        if (strcmp($display, "list") != 0) {
                            $requete = "SELECT id, title, content, creation FROM legacy_chunks WHERE id = '" . $display . "'";
                        } else {
                            if (isset($_GET['tag']) and check($_GET['tag'])) {
                                $list = True;
                                $requete = "SELECT label FROM legacy_tags WHERE tags.id = '" . $_GET['tag'] . "'";
                                $resultat = get($requete);
                                $temp = mysql_fetch_assoc ($resultat);
                                $catname = $temp["label"];
                                //
                                $requete = "SELECT id, title, content, creation FROM legacy_chunks, legacy_tagged WHERE legacy_tagged.id_chunk = chunks.id AND legacy_tagged.id_tag = '" . $_GET['tag'] . "'";
                            }
                        }
                    }
                    else
                        $requete = "SELECT id, title, content, creation FROM legacy_chunks WHERE project='news' ORDER BY creation DESC LIMIT 0, 3";
                ...
                <td class="left">
                    ...
                    <div class="invisible" style="border: 1px dotted black; background-color: #F9FAED;">
                        ...
                        <ul class="rubrique">
                            <?php
                            $requete = "SELECT id, label, count(*) as nb FROM legacy_tags, legacy_tagged WHERE legacy_tagged.id_tag = legacy_tags.id GROUP BY legacy_tags.id, legacy_tags.label";
                            if ($resultat = get($requete))
                            {
                                $r = mysql_num_rows($resultat);
                                while ($arr = mysql_fetch_assoc ($resultat))
