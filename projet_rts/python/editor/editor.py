...
class Application:
    ...
    def start_new_map(self, title=None, width=None, height=None):
        ...
        for layname, lay in self.mod.layerHandlers.items():
            ...
            if lay_cont != 'int' and def_code != 0:
                def_num = self.mod.resources[lay_cont][def_code][0] # at this step, contains (val, file)
            else:
                def_num = def_code
    ...
    def menu_change_visibilty(self, index=None, value=None, op=None, origin='var'):
    ...
    def load_mod_graphics(self):
        ...
        for resname, rescontent in self.mod.resources.items():
            for objname, objcontent in rescontent.items():
                ...
                self.mod.resources[resname][objname] = SimpleImage(os.path.join(self.mod.mod_data['graphics_dir'], file))
        ...
        for x in range(start_x, end_x):
            for y in range(start_y, end_y):
                if 0 <= x < self.map.width and 0 <= y < self.map.height:
                    ...
                    self.map.set(y, x, val, self.mod.layer.name)
